...
public class ListaContactosGSON extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lista.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                Toast.makeText(ListaContactosGSON.this,
                        "Móvil: " + persona.getContactos().get(position).getTelefono().getMovil(),
                        Toast.LENGTH_SHORT
                ).show();
            }
        });
    ...
    private void descarga(String web) {
        ...
        RestClient.get(web, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                progreso.dismiss();
                try {
                    persona = Analisis.analizarContactosGSON(response);
                    Toast.makeText(ListaContactosGSON.this, "Descarga con éxito", Toast.LENGTH_SHORT).show();
                    mostrar();
    ...
    private void mostrar() {
        if (persona != null) {
            if (adapter == null) {
                adapter = new ArrayAdapter<Contacto>(this, android.R.layout.simple_list_item_1, persona.getContactos());
                lista.setAdapter(adapter);
            }
            else {
                adapter.clear();
                adapter.addAll(persona.getContactos());
            }
        } else
